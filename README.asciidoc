= ARK: Survival Ascended Linux Server Tools

This tool allows you to manage any ARK: Survival Ascended linux server without having to remember long commands.
...
* edit `asamanager` wherever it is installed, and replace
+
...
ASA Server Tools requires certain packages be installed in order to function correctly.
...
See https://github.com/Norwegian-Vikings/asa-server-tools/wiki/Install-of-required-versions-of-glibc-and-gcc-on-RHEL-CentOS

To use ASA Server Tools, you will need to install **SteamCMD** following this guide:

https://developer.valvesoftware.com/wiki/SteamCMD#Linux

We assume you have created the `steam` user to store steamcmd and your ASA server.

Requirements
~~~~~~~~~~~~

To allow your ASA server to communicate with the outside you have to open some ports in your firewall:
...
NB: Change the ports according to the ones set in your `asamanager.cfg` file.

Install ASA Server Tools
~~~~~~~~~~~~~~~~~~~~~~~~

To install ASA Server Tools run this command:

[source,sh]
curl -sL https://raw.githubusercontent.com/Norwegian-Vikings/asa-server-tools/master/netinstall.sh | sudo bash -s steam

NB: You may want to change the `bash -s` parameter to fit your steam user if different from `steam`.

If you want to install the `asamanager` service, append the `--install-service` option to the install command.

This will copy the `asamanager` script and its daemon to the proper directories and will create an empty log directory in `/var/log` for ASA Server Tools.

To instead perform a user-install of ASA Server Tools as an unprivileged user, run this command:

[source,sh]
curl -sL https://raw.githubusercontent.com/Norwegian-Vikings/asa-server-tools/master/netinstall.sh | bash -s -- --me

Configuration
~~~~~~~~~~~~~

Stored in `/etc/asamanager/asamanager.cfg` and `/etc/asamanager/instances/<instancename>.cfg` you can find the variables needed to start the server, like the port numbers, the system environment variables and so on.
...
Also, in these files, you can specify any parameter you want to add to the startup command of ASA server.
These parameters must be prefixed by the `ark_` string, some example could be:

[source,sh]
ark_SessionName="My ASA server"
ark_MaxPlayers=50
ark_ServerPVE=False
ark_DifficultyOffset=1

Your session name may not contain special characters (eg. `!![EU]!! Aw&some ASA`) as it could break the startup command.
...
You can override or add variables for a specific system user creating a file called `.asamanager.cfg` in the home directory of the system user.
...
To install ASA Server just run this command as normal user:
...
`asamanager [Global Options] <command> [Command Options] [Command Args] [Instances]`
...
[horizontal]
`upgrade-tools`::
    Upgrades the ASA server tools to the latest version

`uninstall-tools`::
    Uninstalls the ASA server tools
...
`update`::
    Updates an install of the ASA server (or installs it if it's
    ...
    `--validate`;;
        Re-validates the ASA server install
...
`checkupdate`::
    Checks if an ASA server update is available
...
`status`::
    Prints the status of the ASA server
...
Global configuration is stored in `/etc/asamanager/asamanager.cfg`
and/or `~/.asamanager.cfg`.
...
`arkstChannel`::
    ...
    upgrading the ASA server tools
...
`steamcmd_user`::
    ...
    in `~/.asamanager.cfg` in the case of a user-install
...
`arkserverexec`::
    The relative path within an ASA server install where the
    ASA server executable can be found.
+
For the standard ASA server, this should be `ShooterGame/Binaries/Linux/ShooterGameServer`
...
`arkautorestartfile`::
    The relative path within an ASA server install to place the
...
+
default: `/path/to/instance/config.start` where `/path/to/instance/config` is the path to the instance config file with the `.cfg` extension removed - e.g. `/etc/asamanager/instances/main.start`
...
+
default: `/path/to/instance/config.GameUserSettings.ini` where `/path/to/instance/config` is the path to the instance config file with the `.cfg` extension removed - e.g. `/etc/asamanager/instances/main.GameUserSettings.ini`
...
+
default: `/path/to/instance/config.Game.ini` where `/path/to/instance/config` is the path to the instance config file with the `.cfg` extension removed - e.g. `/etc/asamanager/instances/main.Game.ini`
...
`appid`::
    The Steam AppID of the ASA server

`mod_appid`::
    The Steam AppID of the ASA client (used for downloading mods)

`mod_branch`::
    The Mod branch (`Windows` or `Linux`) to use.  Virtually all mods
    use the `Windows` branch for the ASA server, and the `Linux`
...
`/etc/asamanager/instances/<instancename>.cfg`,
`~/.config/asamanager/instances/<instancename>.cfg`
or as specified in the `configfile_<instancename>` options in
the global config.

[horizontal]
`arkserverroot`::
    The directory under which the ASA server is installed
