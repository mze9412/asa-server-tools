...
#
# @LICENSE AGPLv3
# @AUTHOR  Charlie Powell - cdp1337@veraciousnetwork.com, adapted by mze9412
#
# F*** Nitrado
...
apt update
apt install -y software-properties-common apt-transport-https dirmngr ca-certificates curl wget sudo htop
...
#cat > /etc/systemd/system/asa-island.service <<EOF
#[Unit]
...
#User=steam
#Group=steam
#ExecStartPre=/usr/games/steamcmd +force_install_dir /home/steam/asa +login anonymous +app_update 2430930 validate +quit
#WorkingDirectory=/home/steam/asa/ShooterGame/Binaries/Win64
...
#ExecStart=$STEAMDIR/compatibilitytools.d/$PROTON_NAME/proton run ArkAscendedServer.exe TheIsland_WP?listen?Port=40010?RCONEnabled=True?RCONPort=40019?ServerCrosshair=True?AllowThirdPersonPlayer=True?MapPlayerLocation=True?RCONServerGameLogBuffer=1200?NoTributeDownloads=True?PreventDownloadSurvivors=True?PreventDownloadDinos=True?PreventDownloadItems=True?PreventUploadSurvivors=True?PreventUploadDinos=True?PreventUploadItems=True?AllowAnyoneBabyImprintCuddle=True?OverrideOfficialDifficulty=5.0?AllowCrateSpawnsOnTopOfStructures=False?bAllowPlatformSaddleMultiFloors=True?SupplyCrateLootQualityMultiplier=1.5?FishingLootQualityMultiplier=1.5?bAllowUnlimitedRespecs=True?PreventSpawnAnimations=True?CraftingSkillBonusMultiplier=1.5?DestroyUnconnectedWaterPipes=True?FastDecayUnsnappedCoreStructures=True?UseOptimizedHarvestingHealth=True?CrossARKAllowForeignDinoDownloads=True?PvEAllowStructuresAtSupplyDrops=False?ShowFloatingDamageText=True?AllowRaidDinoFeeding=True?TribeLogDestroyedEnemyStructures=True?PreventOfflinePvP=True?PreventOfflinePvPInterval=30?EnableExtraStructurePreventionVolumes=True?ForceAllStructureLocking=true?ServerPassword=?ServerAdminPassword=Gp4hGhGH -servergamelog -servergamelogincludetribelogs -ServerRCONOutputTribeLogs -NoTransferFromFiltering -ShowFloatingDamageText -UseDynamicConfig -SecureSendArKPayload -ClusterDirOverride=/home/steam/clusters -clusterid=NorwegianVikingsAsa -WinLiveMaxPlayers=50 -UseBattlEye
#Restart=on-failure
#RestartSec=20s
...
#   ln -s "/home/steam/asa/ShooterGame/Saved/Config/WindowsServer/GameUserSettings.ini" /home/steam/island-GameUserSettings.ini

# [ -e "/home/steam/island-ShooterGame.log" ] || \
#   ln -s "/home/steam/asa/ShooterGame/Saved/Logs/ShooterGame.log" /home/steam/island-ShooterGame.log
